...
@Component
public class SecondValve extends AbstractValve {
    @Override
    public void doInvoke(ValveContext context) {
        ...
        System.out.println("第二道阀门: requestId-->【" + requestId + "】");

        Map<String,Object> request = context.getRequest();
        if(request.containsKey("source")){
            String value =  request.get("source") + "|" + SecondValve.class.getSimpleName();
