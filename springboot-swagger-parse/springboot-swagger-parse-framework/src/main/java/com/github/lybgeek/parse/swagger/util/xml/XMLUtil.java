package com.github.lybgeek.parse.swagger.util.xml;
...
public class XMLUtil {
    ...
    public static Document stringToDocument(String xml) {
        try {
            return XMLHelpler.getDocument(new ByteArrayInputStream(xml.getBytes(StandardCharsets.UTF_8.name())));
        } catch (Exception e) {
            log.error("错误：", e);
            return null;
        }
        ...
        if (!listElement.isEmpty()) {
            ...
            for (Element e : listElement) {// 遍历所有一级子节点
                ...
                if (!e.attributes().isEmpty()) {
                    ...
                    jsonObject.put("attribute", attributeJson);
                }
                list.add(jsonObject);
            }
            if (list.size() == 1) {
    ...
    public static String jsonToPrettyXml(JSONObject json) {
        ...
        try {
            ...
            log.error("This object can not convert to xml", e);
        }
